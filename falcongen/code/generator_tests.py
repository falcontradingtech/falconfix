...
def _render_roundtrip_cpp(fix_version: str, namespace: str, suite_name: str, message_names: list[str]) -> str:
    ...
    for message in message_names:
        lines.append(f"TEST({suite_name}, {message}_MatchesGeneratedSample) {{")
...
def _render_samples_validation_cpp(
    ...
    lines.append("} // namespace")
    lines.append("")
    lines.append(f"TEST({suite_name}, FixSamplesHaveBasicFixFields) {{")
    ...
    lines.append("")
    if has_mdr:
        lines.append(f"TEST({suite_name}, MarketDataRequestRespectsConfiguredGroupRepetitionsInFixString) {{")
        ...
        lines.append("}")
        lines.append("")
        lines.append(f"TEST({suite_name}, MarketDataRequestConstructedObjectMatchesConfiguredGroupRepetitions) {{")
        lines.append(f"    constexpr int reps = {group_repetitions};")
        ...
        lines.append(f"TEST({suite_name}, MarketDataRequestNotPresentInThisFixVersion) {{")

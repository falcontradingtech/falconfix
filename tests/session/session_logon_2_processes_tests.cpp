...
TEST(FIXSessionTests, ClientServerLogonHandshakeTwoThreads) {
    ...
    FFStatus serverStartRc = FF_OK();
    FFStatus clientStartRc = FF_OK();
    std::thread serverThread([&]() {
        serverStartRc = serverEngine.start();
    });
    serverThread.join();
    ASSERT_TRUE(serverStartRc.ok()) << falconfix::errors::format_error(serverStartRc);
    std::thread clientThread([&]() {
        clientStartRc = clientEngine.start();
    });
    clientThread.join();
    ASSERT_TRUE(clientStartRc.ok()) << falconfix::errors::format_error(clientStartRc);
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (std::chrono::steady_clock::now() < deadline) {
        if (serverApp.onLogonCount.load() > 0 && clientApp.onLogonCount.load() > 0) {
            break;
        }
    ...
    std::thread clientStopThread([&]() {
        clientEngine.stop();
    });
    clientStopThread.join();

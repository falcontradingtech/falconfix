...
// Component: PosUndInstrmtGrp::NoUnderlyings (FIX4.4)
...
TEST_F(FIX4_4_PosUndInstrmtGrp_NoUnderlyingsComponentTest, EncodeDecodeRoundtrip) {
    ...
    // Decode back
    PosUndInstrmtGrp::NoUnderlyings decoded;
    const char *q = buffer;
    bool decode_result = decoded.decode(q, p);
    EXPECT_TRUE(decode_result);
    EXPECT_EQ(decoded.getUnderlyingSettlPrice(), component.getUnderlyingSettlPrice());

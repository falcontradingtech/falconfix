...
using namespace fix42::serialize;

class FIX4_2_SerializerWriteUtcTimeOnlyTests : public ::testing::Test {
protected:
	char buffer[32]{};
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithMilliseconds_ZeroValue) {
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithoutMilliseconds_ZeroValue) {
	std::ptrdiff_t advance = 0;
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithMilliseconds_TypicalValue) {
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithoutMilliseconds_TypicalValue) {
	const int64_t value = 12LL * 10000000 + 34LL * 100000 + 56LL * 1000 + 789LL;
	std::ptrdiff_t advance = 0;
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithMilliseconds_MaxValidTime) {
	// 23:59:59.999
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithoutMilliseconds_MaxValidTime) {
	const int64_t value = 23LL * 10000000 + 59LL * 100000 + 59LL * 1000 + 999LL;
	std::ptrdiff_t advance = 0;
	const auto result = writeAndCapture(value, false, advance);
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithMilliseconds_SingleDigitComponents) {
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, WithMilliseconds_MillisecondsAllTensDigit) {
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, DefaultParameter_IncludesMilliseconds) {
...
TEST_F(FIX4_2_SerializerWriteUtcTimeOnlyTests, PointerAdvancesCorrectly_WhenChained) {
...
class FIX4_2_SerializerPresenceBitmapTests : public ::testing::Test {
protected:
	std::array<uint64_t, 2> bitmap{};
};

TEST_F(FIX4_2_SerializerPresenceBitmapTests, IsPresent_DefaultsToFalse) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, SetPresence_FirstBitOfFirstWord) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, SetPresence_LastBitOfFirstWord) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, SetPresence_FirstBitOfSecondWord) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, SetPresence_LastBitOfSecondWord) {
	set_presence(bitmap, 127);
	EXPECT_TRUE(is_present(bitmap, 127));
	EXPECT_EQ(bitmap[1], (uint64_t(1) << 63));
}

TEST_F(FIX4_2_SerializerPresenceBitmapTests, SetPresence_DoesNotAffectOtherBits) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, ClearPresence_ClearsSetBit) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, ClearPresence_OnAlreadyClearBit_IsNoOp) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, ClearPresence_DoesNotAffectOtherBits) {
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, ClearPresence_LastBitOfSecondWord) {
	set_presence(bitmap, 127);
...
TEST_F(FIX4_2_SerializerPresenceBitmapTests, MultipleSetAndClear_RoundTrip) {
	for (std::size_t i = 0; i < 128; ++i) {
